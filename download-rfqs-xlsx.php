...
try {
    ...
    $xlsxBytes = file_get_contents($xlsxPath);
    @unlink($xlsxPath);

    if ($xlsxBytes === false) {
        throw new RuntimeException('The workbook was created, but could not be read for download.');
    }

    // Send the workbook directly to the browser as a real XLSX attachment.
