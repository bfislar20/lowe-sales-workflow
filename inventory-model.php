...
function im_build(): array {
    ...
    return ['cache_version'=>4,'source_file'=>basename(ld_master()),'source_mtime'=>@filemtime(ld_master())?:0,'as_of'=>$asOf,'inventory_snapshot_date'=>$today,'rows'=>$rows];
}

$cacheFile=__DIR__.'/inventory-dashboard-data.json';$mtime=@filemtime(ld_master())?:0;$data=null;
if(is_file($cacheFile)){$tmp=json_decode((string)@file_get_contents($cacheFile),true);if(is_array($tmp)&&(int)($tmp['cache_version']??0)===4&&(int)($tmp['source_mtime']??-1)===$mtime&&!empty($tmp['rows']))$data=$tmp;}
